	...
	passport.deserializeUser((id, done) => {
		db.getConnection((err: any, connection: any) => {
			if (err) {
				...
				connection.query('SELECT * FROM '+config.db_tables.USERS+' WHERE username = ?', [id], (err: any, rows: any) => {
					connection.release();
			...
			(req: any, username: string, password: string, done: any) => {
				...
				db.getConnection((err: any, connection: any) => {
					if (err) {
						...
						connection.query('SELECT * FROM '+config.db_tables.USERS+' WHERE username = ?', [username], (err: any, rows: any) => {
							if (err) {
								console.error('error', err);
								return done(err);
							}
							...
							if (rows.length) {
								...
								let insertQuery = 'INSERT INTO '+config.db_tables.USERS+' ( username, password ) values (?,?)';
